...
/* ============================================================
   v4.1 GLOBAL FALLBACK (独立作用域，不依赖任何 IIFE 内部函数)
   每 400ms 检查人物详情 4 个 section 是否就绪 → 自动挂载 4 Tab
   ============================================================ */
(function(){
  var TABS41 = [
    {key:'stats',     label:'提问统计',  icon:'📊'},
  ...
  function mountTabs41Once() {
    ...
    var sections = [qa, dp, sp, me];
    for (var i = 0; i < sections.length && i < TABS41.length; i++) {
      sections[i].classList.add('person-section', 'v4-p-section', 'v4-p-' + TABS41[i].key);
      sections[i].setAttribute('data-tab-key', TABS41[i].key);
    }
    var bio = pdp.querySelector(':scope > .person-bio');
    var tabsWrap = document.createElement('div');
    tabsWrap.className = 'v4-p-tabs';
    TABS41.forEach(function(t){
      var b = document.createElement('button');
      b.type = 'button'; b.className = 'v4-p-tab';
      b.setAttribute('data-tab-key', t.key);
      b.innerHTML = '<span class="ic">' + t.icon + '</span><span class="lb">' + t.label + '</span>';
    ...
    tabsWrap.addEventListener('click', function(ev){
      ...
      var idx = -1; for (var k=0;k<TABS41.length;k++) if (TABS41[k].key===key) idx=k;
  ...
  function startPoll41() {
    poll41 = 0;
    if (timer41) { clearInterval(timer41); timer41 = null; }
    timer41 = setInterval(function(){
      var done = mountTabs41Once();
      if (done) { clearInterval(timer41); timer41 = null; return; }
      poll41++;
      if (poll41 > 60) { clearInterval(timer41); timer41 = null; }
    }, 400);
  }
  startPoll41();
  // 点击人物 Tab 时重置轮询
  document.addEventListener('click', function(e){
    if (e.target.closest('.nav-tab[data-tab="persons"]')) startPoll41();
  }, true);
  // 切换人物后重置轮询
  document.addEventListener('click', function(e){
    var it = e.target.closest('.person-list-item');
    var pc = e.target.closest('.person-drawer .pd-cell');
    if (it || pc) setTimeout(startPoll41, 400);
  }, true);
})();
