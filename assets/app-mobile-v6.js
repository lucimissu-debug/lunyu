...
(function () {
  ...
  function switchTab(tabName) {
    // v6：只允许 random / filter 两种（人物 Tab 已删除，链接到人物的全部降级为「漫步」）
    if (tabName !== 'random' && tabName !== 'filter') tabName = 'random';
    App.currentTab = tabName;
    $$('.nav-tab').forEach(el => {
      el.classList.toggle('active', el.dataset.tab === tabName);
    });
    $$('.page-section').forEach(el => {
      // v6：只激活 random/filter，section-persons 永远不激活（CSS 也会隐藏）
  ...
  function parseHash() {
    ...
    // person:xxx → v6 降级为漫步（人物 Tab 已删除）
...
/* ============================================================
   v5 方案A GLOBAL FALLBACK（完全独立作用域，不依赖任何 IIFE 内部函数）
   架构：
   1. 顶部：导航栏(107px fixed) + 人物吸顶条(55px fixed)
   2. Hero：person-header(压缩到 ~100px) + person-bio(3行 clamp + 展开按钮 ~80px)
   3. 内容区：v5-p-scroll-wrap（动态计算高度，内部 overflow-y:auto）
      - 4 个 .person-section 默认隐藏，只有 .active 显示
   4. 底部：v5-p-bottom-tabs（4 Tab fixed 底部 + safe-area）
   完全不使用 sticky！
   ============================================================ */
(function(){
  var V5T = [
  ...
  // ===== 校正 body.v3-persons-active class =====
  function fixBodyClass() {
    try {
      var sp = document.getElementById('section-persons');
      var act = !!(sp && sp.classList.contains('active'));
      document.body.classList.toggle('v3-persons-active', act);
    } catch (e) {}
  }

  // ===== 给 person-bio 包一层 + 加展开/收起按钮 =====
  function mountBioToggle(pdp) {
    try {
      var bio = pdp.querySelector(':scope > .person-bio');
      if (!bio) return;
      if (bio.querySelector('.bio-text') || bio.querySelector('.bio-toggle')) return; // 已处理
      // 把 bio 的全部内容（文本 + 子节点）搬到一个 .bio-text 里
      var bioText = document.createElement('div');
      bioText.className = 'bio-text';
      while (bio.firstChild) bioText.appendChild(bio.firstChild);
      bio.appendChild(bioText);
      // 只有当实际高度 > 3 行估算高（约 58px）时才显示展开按钮
      setTimeout(function(){
        try {
          var h = bioText.getBoundingClientRect().height;
          if (h > 62) {
            var btn = document.createElement('button');
            btn.type = 'button';
            ...
            btn.addEventListener('click', function(ev){
              ...
              bio.classList.toggle('bio-expanded');
              btn.innerHTML = bio.classList.contains('bio-expanded') ? '收起简介 ▴' : '展开简介 ▾';
              // 展开/收起后重新计算内容区高度
              setTimeout(resizeScrollWrap, 20);
              return false;
            }, false);
            bio.appendChild(btn);
            setTimeout(resizeScrollWrap, 50);
          }
        } catch(e){}
      }, 30);
    } catch (e) {}
  ...
  // ===== 创建/校正 v5-p-scroll-wrap 滚动容器 =====
  function ensureScrollWrap(pdp) {
    var qa = pdp.querySelector(':scope > #stat-strip-qa');
    var dp = pdp.querySelector(':scope > #stat-strip-dp');
    var sp = pdp.querySelector(':scope > #block-spoken');
    var me = pdp.querySelector(':scope > #block-mentioned');
    if (!qa || !dp || !sp || !me) return null;

    // 1) 如果 4 个 section 已经在 v5-p-scroll-wrap 里了，直接返回
    var existingWrap = pdp.querySelector(':scope > .v5-p-scroll-wrap');
    if (existingWrap && existingWrap.contains(qa) && existingWrap.contains(dp) && existingWrap.contains(sp) && existingWrap.contains(me)) {
      // 校正 4 secs class
      var arr = [qa, dp, sp, me];
      for (var j = 0; j < arr.length && j < V5T.length; j++) {
        var el = arr[j];
        el.className = el.className.replace(/v4-p-[a-zA-Z0-9_-]*/g, '').trim();
        el.classList.remove('person-section','v4-p-section');
        el.classList.add('person-section','v4-p-section','v4-p-'+V5T[j].key);
        el.setAttribute('data-tab-key', V5T[j].key);
      }
      return existingWrap;
    }

    // 2) 移除旧的 v5-p-scroll-wrap（如果存在但没包含 4 secs）
    if (existingWrap) existingWrap.parentNode.removeChild(existingWrap);

    // 3) 创建 wrap
    var wrap = document.createElement('div');
    wrap.className = 'v5-p-scroll-wrap';
    wrap.setAttribute('data-v5-wrap','1');

    // 4) 给 4 secs 校正 class，然后移进 wrap
    var arr = [qa, dp, sp, me];
    var firstSec = qa; // 用于定位插入位置
    for (var j = 0; j < arr.length && j < V5T.length; j++) {
      var el = arr[j];
      el.className = el.className.replace(/v4-p-[a-zA-Z0-9_-]*/g, '').trim();
      el.classList.remove('person-section','v4-p-section');
    ...
    // 5) 插在 bio 后面（4 secs 原来的位置）
    var bio = pdp.querySelector(':scope > .person-bio');
    if (bio && bio.nextSibling) pdp.insertBefore(wrap, bio.nextSibling);
    else if (firstSec && firstSec.nextSibling) pdp.insertBefore(wrap, firstSec);
    else pdp.appendChild(wrap);

    return wrap;
  }

  // ===== 创建/校正 底部 4 Tab =====
  function ensureBottomTabs(pdp) {
    // 底部 Tab 不挂在 pdp 里，挂在 body 末尾（fixed 定位，全局唯一）
    var existing = document.querySelector('.v5-p-bottom-tabs');
    if (existing) {
      // 检查是否 4 Tab，否则重建
      var bs = existing.querySelectorAll('.v5-p-tab');
      if (bs.length === 4 && existing.getAttribute('data-v5-bound') === '1') {
        return existing;
      }
      existing.parentNode.removeChild(existing);
    }
    var tabsBar = document.createElement('div');
    tabsBar.className = 'v5-p-bottom-tabs';
    V5T.forEach(function(t){
      var b = document.createElement('button');
      b.type = 'button'; b.className = 'v5-p-tab';
      b.setAttribute('data-tab-key', t.key);
      b.innerHTML = '<span class="ic">' + t.icon + '</span><span class="lb">' + t.label + '</span>';
      tabsBar.appendChild(b);
    });
    // 逐个绑定 click handler（cloneNode 替换，最干净）
    var btns = tabsBar.querySelectorAll('.v5-p-tab');
    btns.forEach(function(btn){
      ...
      newBtn.addEventListener('click', function(ev){
        ev.preventDefault(); ev.stopPropagation();
        var idx = -1;
        for (var k=0;k<V5T.length;k++) if (V5T[k].key === key) idx = k;
        var allT = tabsBar.querySelectorAll('.v5-p-tab');
        allT.forEach(function(tt){ tt.classList.toggle('active', tt.getAttribute('data-tab-key') === key); });
        var allS = document.querySelectorAll('.v5-p-scroll-wrap .person-section');
        allS.forEach(function(s, j){ s.classList.toggle('active', j === idx); });
        // 切 Tab 后，把内容区滚回顶部
        var wrap = document.querySelector('.v5-p-scroll-wrap');
        if (wrap) try { wrap.scrollTop = 0; } catch(e){}
        // 触发 ECharts resize（如果里面有图表）
        try { if (window.dispatchEvent) window.dispatchEvent(new Event('resize')); } catch(e){}
        return false;
      }, false);
      btn.parentNode.replaceChild(newBtn, btn);
    });
    tabsBar.setAttribute('data-v5-bound', '1');
    document.body.appendChild(tabsBar);
    return tabsBar;
  }

  // ===== 动态计算内容区高度 =====
  function resizeScrollWrap() {
    if (!document.body.classList.contains('v3-persons-active')) return;
    var wrap = document.querySelector('.v5-p-scroll-wrap');
    if (!wrap) return;
    try {
      var nav = document.querySelector('.main-nav-mobile, .top-nav, nav[class*="nav"]');
      var subNav = document.querySelector('.person-sub-nav');
      var pHeader = document.querySelector('.person-detail-panel .person-header');
      var pBio = document.querySelector('.person-detail-panel .person-bio');
      var tabsBottom = document.querySelector('.v5-p-bottom-tabs');

      // 用 getBoundingClientRect 真实高度（比 CSS 估计准）
      var navH = nav ? nav.getBoundingClientRect().height : 107;
      var subH = subNav ? subNav.getBoundingClientRect().height : 55;
      var headerH = pHeader ? pHeader.getBoundingClientRect().height : 100;
      var bioH = pBio ? pBio.getBoundingClientRect().height : 80;
      var tabsH = tabsBottom ? tabsBottom.getBoundingClientRect().height : 56;

      // 视口高度：100dvh（iPhone 动态视口）
      var dvh = window.innerHeight;
      var used = navH + subH + headerH + bioH + tabsH;
      var avail = Math.max(120, dvh - used - 2); // -2 视觉呼吸
  ...
  function mountV5Once() {
    try {
      ...
      // 1) Hero 压缩：bio 展开/收起按钮
      mountBioToggle(pdp);

      // 2) 内容滚动容器
      ensureScrollWrap(pdp);

      // 3) 底部 Tab
      ensureBottomTabs(pdp);

      // 4) 激活第一个（如果还没激活）
      var tbs = document.querySelectorAll('.v5-p-bottom-tabs .v5-p-tab');
      var scs = document.querySelectorAll('.v5-p-scroll-wrap .person-section');
      var hasActiveTab = false;
      tbs.forEach(function(t){ if (t.classList.contains('active')) hasActiveTab = true; });
      var hasActiveSec = false;
      scs.forEach(function(s){ if (s.classList.contains('active')) hasActiveSec = true; });
      if (tbs.length > 0 && !hasActiveTab) tbs[0].classList.add('active');
      if (scs.length > 0 && !hasActiveSec) scs[0].classList.add('active');

      // 5) 动态算高度（DOM 全渲染后再算一次，更准）
      setTimeout(resizeScrollWrap, 10);
      setTimeout(resizeScrollWrap, 100);
      setTimeout(resizeScrollWrap, 400);

      return true;
    } catch (e) {
      console.error('[V5A] mount error:', e && e.message);
      return false;
    }
  }

  // ===== 轮询 + 事件 =====
  var pollCnt = 0, pollTm = null;
  function startPoll() {
    pollCnt = 0;
    if (pollTm) { clearInterval(pollTm); pollTm = null; }
    pollTm = setInterval(function(){
      pollCnt++;
      var done = mountV5Once();
      if (done) { clearInterval(pollTm); pollTm = null; return; }
      if (pollCnt > 80) { clearInterval(pollTm); pollTm = null; }
    }, 200);
  }

  // 启动：DOMContentLoaded + load + 1s 兜底
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', startPoll, false);
    window.addEventListener('load', function(){ setTimeout(startPoll, 200); }, false);
  } else {
    startPoll();
    window.addEventListener('load', function(){ setTimeout(startPoll, 200); }, false);
  }

  // 点击 nav-tab（切换 Tab）：校正 class + 重挂载
  document.addEventListener('click', function(e){
    if (e.target.closest('.nav-tab')) {
      setTimeout(function(){ fixBodyClass(); startPoll(); resizeScrollWrap(); }, 150);
    }
    var it = e.target.closest('.person-list-item');
    var pc = e.target.closest('.person-drawer .pd-cell');
    if (it || pc) setTimeout(function(){ startPoll(); resizeScrollWrap(); }, 400);
  }, true);
  ...
  window.addEventListener('resize', function(){
    ...
    resizeTm = setTimeout(resizeScrollWrap, 80);
  }, false);
  window.addEventListener('orientationchange', function(){
    setTimeout(resizeScrollWrap, 200);
  }, false);

  // 每秒兜底：校正 body class + 重算高度
...
/* ============================================================
   v5.1 GLOBAL (完全重写：杀轮询 + 懒渲染 + 全局事件委托 + ECharts批处理
   架构原则：
   1) 0 个 setInterval（除了 5 秒 1 次的极端兜底，5秒=轻量 DOM 查询，不做修改）
   2) 4 Tab 只渲染当前 active（其他 3 个：不是 display:none，是 DOM 里根本没有节点）
   3) 全局 click 事件委托（不 cloneNode 重绑，不分别给每个 button 单独绑定）
   4) ECharts resize 统一 rAF 批处理
   ============================================================ */
(function(){
  'use strict';
  // 防止旧 IIFE 互相打架：立个 flag
  window.__V51_ACTIVE = true;
  ...
  // 4 section 的 DOM id（按上面 T 对照顺序）：
  var SEC_BY_KEY = {
    stats:     {id:'stat-strip-qa', cls:'stat-strip', clsOrig:null},
    dialog:    {id:'stat-strip-dp', cls:'stat-strip'},
    speeches:  {id:'block-spoken',    cls:'collection-block'},
    mentioned: {id:'block-mentioned', cls:'collection-block'}
  };
  var SEC_ORDER = ['stats','dialog','speeches','mentioned'];
  // 当前人物名：记录上次渲染的人物名（用于判断是否需要重新构建内容）
  var STATE = {
    personName: null,
    // 4 Tab 内容缓存：{personName: {tabKey: htmlString_or_nodeClone}
    cache: {},
    tab: 'stats'
  };
  ...
  function log(msg) {
    ...
    var t = now();
    if (t - LAST_LOG < 50) return; // 50ms 限频，防刷 log 烧性能
    LAST_LOG = t;
    console.log('[V51] ' + msg);
  }

  // ===========================================
  // S1. body class 校正（事件驱动，不轮询
  // ===========================================
  function fixBodyClass() {
    try {
      var sp = document.getElementById('section-persons');
  ...
  // ===========================================
  // S2. 底部 Tab 栏创建（只创建一次，永远不重建）
  // ===========================================
  function ensureBottomTabs() {
    var exist = document.querySelector('.v5-p-bottom-tabs');
    if (exist) return exist;
    var bar = document.createElement('div');
    bar.className = 'v5-p-bottom-tabs';
    TAB.forEach(function(t){
      var b = document.createElement('button');
      b.type = 'button';
      b.className = 'v5-p-tab';
      b.setAttribute('data-v51-tab', t.key);
      b.innerHTML = '<span class="ic">'+t.icon+'</span><span class="lb">'+t.label+'</span>';
      bar.appendChild(b);
    });
  ...
  // ===========================================
  // S3. bio 展开/收起按钮（只创建一次）
  // ===========================================
  function ensureFactAttrs(pdp) {
    var facts = qsa('.person-header .person-fact', pdp);
    facts.forEach(function(f){
      if (f.getAttribute('data-v51-fact')) return;
      var lbl = f.querySelector('.label');
      if (!lbl) return;
      var name = lbl.textContent.trim();
      if (name) f.setAttribute('data-v51-fact', name);
    });
  }
  function ensureBioToggle(pdp) {
    ensureFactAttrs(pdp);
    var bio = qs(':scope > .person-bio', pdp);
    if (!bio) return;
    // 如果已经处理过（有 bio-text 就直接返回
    if (bio.querySelector('.bio-text')) return;
    var bioText = document.createElement('div');
    bioText.className = 'bio-text';
    while (bio.firstChild) bioText.appendChild(bio.firstChild);
    bio.appendChild(bioText);
    ...
    requestAnimationFrame(function(){
      try {
        ...
        if (h > 62) {
          ...
          btn.type = 'button';
          btn.className = 'bio-toggle';
          btn.setAttribute('data-v51-bio-toggle','1');
          btn.innerHTML = '展开简介 ▾';
          bio.appendChild(btn);
          scheduleResize();
        }
  ...
  // ===========================================
  // S4. 内容滚动容器（只创建一次，永不重建）
  // 注意：懒渲染：里面只有当前 Tab 的内容，其他根本没有
  // ===========================================
  function ensureScrollWrap(pdp) {
    var exist = qs(':scope > .v5-p-scroll-wrap', pdp);
    if (exist) return exist;
    var wrap = document.createElement('div');
    wrap.className = 'v5-p-scroll-wrap';
    wrap.setAttribute('data-v51-wrap','1');
    // 4 个占位 section：每个都有正确的 id + class + data-tab-key，但是内部 **空的**，没内容
    SEC_ORDER.forEach(function(key, idx){
      var sec = document.createElement('div');
      var info = SEC_BY_KEY[key];
      sec.id = info.id;
      sec.className = info.cls + ' person-section v4-p-section v4-p-'+key;
      sec.setAttribute('data-tab-key', key);
      sec.setAttribute('data-v51-empty','1'); // 标记：还没注入内容
  ...
  // ===========================================
  // S5. 获取当前人物名 + Tab 内容（懒渲染核心）
  //     利用全局存在的全局函数渲染内容函数通常来自 app.js。
  //     策略：如果 DOM 里当前人物的内容已经被主 IIFE 渲染过（因为主 IIFE 会在切人时填充 4 个 section 的内容），
  // 我们把已渲染过的内容抽出来，之后再注入。
  //     如果内容还没有，说明主 IIFE 还没跑，等它跑完再取。
  // ===========================================
  function getCurrentPersonName() {
    var nameEl = qs('.person-detail-panel .person-name');
    if (nameEl && nameEl.textContent) return nameEl.textContent.trim();
    return null;
  }

  // 把 wrap 里的 section（主 IIFE 生成好的内容）缓存起来
  function captureExistingContentIntoCache() {
    var pName = getCurrentPersonName();
    if (!pName) return false;
    var pdp = qs('.person-detail-panel');
    if (!pdp) return false;
    var wrap = qs(':scope > .v5-p-scroll-wrap', pdp);
    if (!wrap) return false;
    if (!STATE.cache[pName]) STATE.cache[pName] = {};

    var any = false;
    SEC_ORDER.forEach(function(key){
      var info = SEC_BY_KEY[key];
      var sec = qs(':scope > #'+info.id, wrap);
      if (sec && sec.innerHTML && sec.innerHTML.length > 20 && sec.getAttribute('data-v51-empty') !== '1' && !STATE.cache[pName][key]) {
        // 存 HTML string（不存 live node，省内存）
        STATE.cache[pName][key] = sec.innerHTML;
        any = true;
      }
    });
    return any;
  ...
  // 根据 key 注入内容到 section
  function injectTabContent(tabKey) {
    var pName = getCurrentPersonName();
    if (!pName) return false;
    var pdp = qs('.person-detail-panel');
    if (!pdp) return false;
    var wrap = qs(':scope > .v5-p-scroll-wrap', pdp);
    if (!wrap) return false;
    var info = SEC_BY_KEY[tabKey];
    if (!info) return false;
    var sec = qs(':scope > #'+info.id, wrap);
    if (!sec) return false;

    // 1. 如果已经有内容（data-v51-empty != 1）直接返回
    if (sec.getAttribute('data-v51-empty') !== '1') return true;

    // 2. 先看缓存
    if (STATE.cache[pName] && STATE.cache[pName][tabKey]) {
      sec.innerHTML = STATE.cache[pName][tabKey];
      sec.removeAttribute('data-v51-empty');
      scheduleResize();
      return true;
    }

    // 3. 缓存没有：等主 IIFE 填充完再抓（主 IIFE 在切人后在 4 个 section 一次性填完，我们直接取
    //    如果现在 wrap 外面有没有被主 IIFE 先填过的内容（在 wrap 外面可能有 4 个原始 section（v4.2 之前的做法）
    //    如果没有，让它「下次再（懒（等用户真需要时再触发一下 mountV51Once 函数会重新触发主 IIFE 重新填充
    return false;
  }
  ...
  function switchTab(tabKey) {
    // 1. 底部 Tab 高亮
    var bar = document.querySelector('.v5-p-bottom-tabs');
    if (bar) {
      qsa('.v5-p-tab', bar).forEach(function(b){
        b.classList.toggle('active', b.getAttribute('data-v51-tab') === tabKey);
      });
    }
    // 2. 内容 section 切换
    var pdp = qs('.person-detail-panel');
    var wrap = pdp ? qs(':scope > .v5-p-scroll-wrap', pdp) : null;
    if (!wrap) return;
    var secs = qsa(':scope > .person-section', wrap);
    secs.forEach(function(sec){
      sec.classList.toggle('active', sec.getAttribute('data-tab-key') === tabKey);
    });
    // 3. 懒渲染：如果内容还没注入，注入
    injectTabContent(tabKey);
    STATE.tab = tabKey;
    // 4. 滚回顶
    try { wrap.scrollTop = 0; } catch(e){}
    // 5. ECharts resize（rAF 批处理
  ...
  function doResize() {
    ...
    if (!wrap) return;
    try {
      var navH = 0, subH = 0, phH = 0, bioH = 0, tabsH = 56;
      var nav = document.querySelector('.main-nav-mobile, .top-nav');
      var subNav = document.querySelector('.person-sub-nav');
      var ph = document.querySelector('.person-detail-panel .person-header');
      var bio = document.querySelector('.person-detail-panel .person-bio');
      var bTabs = document.querySelector('.v5-p-bottom-tabs');
      if (nav) navH = nav.getBoundingClientRect().height;
      if (subNav) subH = subNav.getBoundingClientRect().height;
      if (ph) phH = ph.getBoundingClientRect().height;
      if (bio) bioH = bio.getBoundingClientRect().height;
      if (bTabs) tabsH = bTabs.getBoundingClientRect().height;
      var used = navH + subH + phH + bioH + tabsH;
      var dvh = window.innerHeight || 600;
      var avail = Math.max(140, dvh - used - 4);
      wrap.style.height = avail + 'px';
      wrap.style.maxHeight = avail + 'px';
    } catch (e) {}
  }
  ...
  // ===========================================
  // S8. ECharts resize 批处理
  // ===========================================
  var ecScheduled = false;
  function scheduleEChartsResize() {
    if (ecScheduled) return;
    ecScheduled = true;
    requestAnimationFrame(function(){
      ecScheduled = false;
      try {
        if (window.echarts && typeof window.dispatchEvent) {
          window.dispatchEvent(new Event('resize'));
        }
      } catch(e1) {
        try { if (window.dispatchEvent) window.dispatchEvent(new Event('orientationchange')); } catch(e2){}
      }
    });
  }
  ...
  // S9. 主挂载函数（事件驱动，不轮询
  // ===========================================
  var MOUNTED_PDP = null;
  var MOUNT_TIMES = 0;
  function mountV51Once() {
    MOUNT_TIMES++;
    if (MOUNT_TIMES > 10) return false; // 最多 10 次硬上限（极端 case 不轮询）
    fixBodyClass();
    var pdp = qs('.person-detail-panel');
    if (!pdp) return false;
    // 4 section 都能找到主 IIFE 有没有填完
    var wrap = ensureScrollWrap(pdp);
    if (!wrap) return false;
    ensureBottomTabs();
    ensureBioToggle(pdp);
    MOUNTED_PDP = pdp;
    ...
    // 捕获内容缓存
    captureExistingContentIntoCache();
    // 如果当前人物变了 → 默认激活 stats 且渲染它
    var pName = getCurrentPersonName();
    if (pName !== STATE.personName) {
      STATE.personName = pName;
      // 新人物：清空 4 个 section 重新标记空（等需要 Tab 点到再懒加载
      qsa(':scope > .person-section', wrap).forEach(function(sec){
        sec.setAttribute('data-v51-empty','1');
      });
    }
    // 激活当前 Tab
  ...
  // ===========================================
  // S10. 全局 click 事件委托（1 个 listener，永不重绑）
  // ===========================================
  var CLICK_LAST = 0;
  document.addEventListener('click', function(e){
    var t = now();
    if (t - CLICK_LAST < 20) return; // 20ms 防抖动（极快速连续点击
    CLICK_LAST = t;
    var et = e.target;

    // 1) 切 nav-tab
    if (et.closest('.nav-tab')) {
      setTimeout(function(){
        fixBodyClass();
        mountV51Once();
        scheduleResize();
    ...
    // 2) 底部 4 Tab 点击
    var tabBtn = et.closest && et.closest('.v5-p-tab[data-v51-tab]');
    if (tabBtn) {
      e.preventDefault();
      e.stopPropagation();
      var k = tabBtn.getAttribute('data-v51-tab');
      if (k) switchTab(k);
      return false;
    }

    // 3) bio 展开/收起
    if (et.closest && et.closest('[data-v51-bio-toggle]')) {
      var bio = et.closest('.person-bio');
      if (bio) {
        bio.classList.toggle('bio-expanded');
        var btn = bio.querySelector('.bio-toggle');
        if (btn) btn.innerHTML = bio.classList.contains('bio-expanded') ? '收起简介 ▴' : '展开简介 ▾';
        scheduleResize();
      }
      e.preventDefault();
      return false;
    ...
    // 4) 切人物：人物抽屉 cell / person-list-item
    var per = (et.closest && et.closest('.person-list-item')) || (et.closest && et.closest('.person-drawer .pd-cell'));
    if (per) {
      // 等主 IIFE 先切完人再
      setTimeout(function(){
        mountV51Once();
    ...
    // 5) 打开/关闭 人物抽屉（兜底校正
    if (et.closest && et.closest('#v4-open-drawer, #v3-drawer-close, .pd-expand-btn')) {
      setTimeout(function(){ scheduleResize(); }, 350);
      return;
    }
  }, true);
  ...
  if (document.readyState === 'loading') {
    ...
    window.addEventListener('load', function(){ setTimeout(start, 200); }, false);
  } else {
    start();
    window.addEventListener('load', function(){ setTimeout(start, 200); }, false);
  }

  // resize / orientationchange：事件驱动，不轮询
  var resizeTm = null;
  window.addEventListener('resize', function(){
    if (resizeTm) clearTimeout(resizeTm);
    resizeTm = setTimeout(function(){ scheduleResize(); scheduleEChartsResize(); }, 120);
  }, false);
  window.addEventListener('orientationchange', function(){
    setTimeout(function(){ scheduleResize(); scheduleEChartsResize(); }, 250);
  }, false);

  // ===========================================
  // S12. 5 秒一次的**只读**极端兜底（只做读操作，不改 DOM）
  //      （用户切 Tab 后主 IIFE 突然改了结构，我们最多 5 秒校正一次
  //      0 次 DOM 修改，除非完全安全，手机上 5 秒 1 次可忽略
  // ===========================================
  setInterval(function(){
    try {
      var act = document.getElementById('section-persons');
      var isPerson = !!(act && act.classList.contains('active'));
      document.body.classList.toggle('v3-persons-active', isPerson);
      if (!isPerson) return;
      var wrap = document.querySelector('.v5-p-scroll-wrap');
      if (!wrap) { mountV51Once(); return; }
      var secs = qsa(':scope > .person-section', wrap);
      if (secs.length !== 4) mountV51Once();
    } catch(e){}
  }, 5000);
